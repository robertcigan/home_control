class Device::Curtain < Device
  ...
  def setup_pin
    send_to_board(add: { id: id, type: "curtain", open_pin: pin, close_pin: pin + 1, inverted: inverted } )
  end
  ...
  def activate_curtain(direction, time) #time in ms
    if direction == "open"
      send_to_board(write: { id: id, action: "open", value: time})
    elsif direction == "close"
      send_to_board(write: { id: id, action: "close", value: time })
    end
  end
