class Device::Relay < Device
  ...
  def setup_pin
    send_to_arduino(add: { id: id, type: "relay", pin: pin, default_value: value, inverted: inverted } )
  ...
  def set_value_to_arduino
    send_to_arduino(write: { id: id, type: "relay", value: value})
  end
  ...
  def detect_change
    if previous_changes.key?(:value_boolean)
      set_value_to_arduino # send to arduino
      trigger_programs
    end
  end
